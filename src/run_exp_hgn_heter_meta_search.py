...
def run_exp_han(dataname="HGB_ACM", seed=0, config=None):
    ...
    if config.rewire_whole:
        ...
        rewirer        = ModelHandler(in_sizes, len(edge_index_dict_rewire), num_labels, device=device, window_size=[config.window_size, config.window_size], num_epoch=1, num_epoch_finetune=1, shuffle=[True, True], thres_min_deg=config.thres_min_deg, thres_min_deg_ratio=config.thres_min_deg_ratio, use_clf=config.use_clf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k",                   type=int,   default=0,          help="growing size. ")
    ...
    if args.dataset == "Actor":
       ...
       #  args.thres_prunning = [0.3, 0.3, 0.3]
        ...
        args.thres_prunning = [-1.0, -0.1, 0.3, 0.6]
