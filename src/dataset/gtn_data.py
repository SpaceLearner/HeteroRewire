...
class GTNDataset(Dataset):
    ...
    def __init__(self, root, name):
        ...
        self.meta_instances = pickle.load(open(self.processed_paths[1], 'rb'))
    ...
    def processed_file_names(self):
        return ["data.pt", "meta_instances.pkl"]
    ...
    def read_gtn_data(self, folder):
        ...
        if self.name == 'gtn-acm':
            hetero_graph = dgl.heterograph({
                ('P', '1', 'A'): (torch.tensor(edges[0].nonzero()[0], dtype=torch.long), torch.tensor(edges[0].nonzero()[1], dtype=torch.long)),
                ('A', '2', 'P'): (torch.tensor(edges[1].nonzero()[0], dtype=torch.long), torch.tensor(edges[1].nonzero()[1], dtype=torch.long)),
                ('P', '3', 'S'): (torch.tensor(edges[2].nonzero()[0], dtype=torch.long), torch.tensor(edges[2].nonzero()[1], dtype=torch.long)),
                ('S', '4', 'P'): (torch.tensor(edges[3].nonzero()[0], dtype=torch.long), torch.tensor(edges[3].nonzero()[1], dtype=torch.long))
            })
            print(hetero_graph)
            self.meta_instances['PAP'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('P', '1', 'A'), ('A', '2', 'P')]).edges())
            self.meta_instances['PSP'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('P', '3', 'S'), ('S', '4', 'P')]).edges())
        elif self.name == 'gtn-dblp':
            hetero_graph = dgl.heterograph({
                ('P', '1', 'A'): (torch.tensor(edges[0].nonzero()[0], dtype=torch.long), torch.tensor(edges[0].nonzero()[1], dtype=torch.long)),
                ('A', '2', 'P'): (torch.tensor(edges[1].nonzero()[0], dtype=torch.long), torch.tensor(edges[1].nonzero()[1], dtype=torch.long)),
                ('P', '3', 'C'): (torch.tensor(edges[2].nonzero()[0], dtype=torch.long), torch.tensor(edges[2].nonzero()[1], dtype=torch.long)),
                ('C', '4', 'P'): (torch.tensor(edges[3].nonzero()[0], dtype=torch.long), torch.tensor(edges[3].nonzero()[1], dtype=torch.long))
            })
            print(hetero_graph)
            self.meta_instances['APA']   = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('A', '2', 'P'), ('P', '1', 'A')]).edges())
            self.meta_instances['APAPA'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('A', '2', 'P'), ('P', '1', 'A'), ('A', '2', 'P'), ('P', '1', 'A')]).edges())
            self.meta_instances['APCPA'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('A', '2', 'P'), ('P', '3', 'C'), ('C', '4', 'P'), ('P', '1', 'A')]).edges())
        elif self.name == 'gtn-imdb':
            hetero_graph = dgl.heterograph({
                ('M', '1', 'D'): (torch.tensor(edges[0].nonzero()[0], dtype=torch.long), torch.tensor(edges[0].nonzero()[1], dtype=torch.long)),
                ('D', '2', 'M'): (torch.tensor(edges[1].nonzero()[0], dtype=torch.long), torch.tensor(edges[1].nonzero()[1], dtype=torch.long)),
                ('M', '3', 'A'): (torch.tensor(edges[2].nonzero()[0], dtype=torch.long), torch.tensor(edges[2].nonzero()[1], dtype=torch.long)),
                ('A', '4', 'M'): (torch.tensor(edges[3].nonzero()[0], dtype=torch.long), torch.tensor(edges[3].nonzero()[1], dtype=torch.long))
            })
            print(hetero_graph)
            self.meta_instances['MDM']   = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('M', '1', 'D'), ('D', '2', 'M')]).edges())
            self.meta_instances['MAM']   = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('M', '3', 'A'), ('A', '4', 'M')]).edges())
            self.meta_instances['MDMAM'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('M', '1', 'D'), ('D', '2', 'M'), ('M', '3', 'A'), ('A', '4', 'M')]).edges())
            self.meta_instances['MAMDM'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('M', '3', 'A'), ('A', '4', 'M'), ('M', '1', 'D'), ('D', '2', 'M')]).edges())
            self.meta_instances['MDMDM'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('M', '1', 'D'), ('D', '2', 'M'), ('M', '1', 'D'), ('D', '2', 'M')]).edges())
            self.meta_instances['MAMAM'] = torch.vstack(dgl.metapath_reachable_graph(hetero_graph, [('M', '3', 'A'), ('A', '4', 'M'), ('M', '3', 'A'), ('A', '4', 'M')]).edges())
    ...
    def process(self):
        ...
        pickle.dump(self.meta_instances, open(self.processed_paths[1], 'wb'))
