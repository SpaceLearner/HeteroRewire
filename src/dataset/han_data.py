...
class HANDataset(Dataset):
    ...
    def read_gtn_data(self, folder):
        ...
        if self.name == "han-acm":
            ...
            self.meta_instances['PAP'] = data["PAP"] - np.eye(num_nodes)
            self.meta_instances['PLP'] = data["PLP"] - np.eye(num_nodes)
        elif self.name == "han-dblp":
            rownetworks = [
            ...
            self.meta_instances['APA']   = data["net_APA"] - np.eye(num_nodes)
            self.meta_instances['APCPA'] = data["net_APCPA"] - np.eye(num_nodes)
            self.meta_instances['APTPA'] = data["net_APTPA"] - np.eye(num_nodes)
            ...
            self.meta_instances['MAM'] = data["MAM"] - np.eye(num_nodes)
            self.meta_instances['MDM'] = data['MDM'] - np.eye(num_nodes)
            self.meta_instances['MYM'] = data['MYM'] - np.eye(num_nodes)
