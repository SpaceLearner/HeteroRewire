...
class LINKX(nn.Module):	
    ...
    def __init__(self, g, in_channels, hidden_channels, out_channels, num_layers, num_nodes, dropout=.5, cache=False, inner_activation=False, inner_dropout=False, init_layers_A=1, init_layers_X=1):
        super(LINKX, self).__init__()	

        self.g = g

        self.fc_list = nn.ModuleList([nn.Linear(in_dim, hidden_channels, bias=True) for in_dim in in_channels])
        self.mlpA = MLP(num_nodes, hidden_channels, hidden_channels, init_layers_A, dropout=0)
        self.mlpX = MLP(hidden_channels, hidden_channels, hidden_channels, init_layers_X, dropout=0)
    ...
    def forward(self, features_list):	
        m = self.num_nodes
        hidden = self.feat_encode(features_list)
        # feat_dim = data.graph['node_feat']	
        edge_index = torch.stack(self.g.edges())
        row, col = edge_index # data.graph['edge_index']	
        row = row-row.min()
        ...
        xX = self.mlpX(hidden, input_tensor=True)
