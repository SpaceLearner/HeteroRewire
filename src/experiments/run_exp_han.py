...
def run_exp_han(dataname="HGB_IMDB", seed=0):
    ...
    for idx, key in enumerate(graph.edge_index_dict):
        if "metapath" in key[1] and int(key[1][9:]) in selected_metapath:
            ...
            edge_index_dict[key] = to_undirected(edge_index_dict[key])
    ...
    for idx, key in enumerate(edge_index_dict):
        if int(key[1][9:]) in selected_rewire:
            ...
            Rewirer    = ModelHandler(graph[target].x.shape[1], graph[target].y.max()+1, device=device, window_size=[10000, 10000])
            data_new   = Rewirer(data, cat_self=False, prunning=True, thres_prunning=0.3)
            edge_index_dict[key] = data_new.edge_index
            
    metadata  = ([target], [(target, "metapath_"+str(i), target) for i in selected_metapath])
    model     = HAN(x_dict[target].shape[1], 128, num_labels, 8, num_layers=1, one_hot=True if dataname=="HGB_Freebase" else False, meta_data=metadata).to(device)
...
def main(args):
    ...
    dataname = "HGB_Freebase"
    seeds = [0, 1, 2, 3, 4]
    results = []
    for idx, seed in enumerate(seeds):
        results.append(run_exp_han(dataname, seed))
...
if __name__ == "__main__":
    ...
    wandb.init(project="Hetero", entity="gjyspliter")